...
bearing_probabilities = [0.10, 0.14, 0.24,
                         0.14, 0.12, 0.10, 0.06, 0.05, 0.03, 0.02]
...
for i in range(1, len(bearing_probabilities)):
    cumulative_probabilities.append(
        cumulative_probabilities[i-1] + bearing_probabilities[i])
...
for i in range(1, len(delay_probabilities)):
    cumulative_probabilities.append(
        cumulative_probabilities[i-1] + delay_probabilities[i])
...
try:
    for i in range(maxlen):
        if (bearing_1[i] == bearing_2[i] == bearing_3[i]):
            three_down += 1
            max_delay = max(
                bearing_3_delay[i], bearing_2_delay[i], bearing_1_delay[i])
...
total_repairman_delay_time = sum(
    bearing_1_delay) + sum(bearing_2_delay) + sum(bearing_3_delay)
print("Total repairman delay time =", total_repairman_delay_time, "minutes")
time_spent_in_bearing_replacement = (total_change * one_bearing_change_time_minutes) - (two_down * (two_bearing_change_time_minutes -
                                                                                                    one_bearing_change_time_minutes)) - (three_down * (three_bearing_change_time_minutes - one_bearing_change_time_minutes))
print("Time spent in bearing replacement =",
      time_spent_in_bearing_replacement, "minutes")
...
print("\n")
print("Total estimated cost = Rs.",  cost_of_repairman +
      cost_of_bearing+cost_of_downtime)
